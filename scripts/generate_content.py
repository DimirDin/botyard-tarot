...
Wave 2 (TODO, partial proof-of-schema for ~15 cards): finance, health, spirit
...
# Wave 2 themes: fully done for these 15 cards only (proof of schema)
WAVE2_CARDS = {
    "major_00_fool", "major_01_magician", "major_03_empress", "major_09_hermit",
    "major_13_death", "major_16_tower", "major_17_star", "major_19_sun",
    "minor_pentacles_ace", "minor_pentacles_10", "minor_cups_02", "minor_swords_03",
    "minor_wands_king", "minor_cups_queen", "major_21_world",
}

TODO_STUB = "TODO (волна 2): базовый текст ещё не написан для этой темы+карты. Смотрите WAVE2_CARDS в scripts/generate_content.py."
...
def build_card_json(card, idx):
    ...
    if card["id"] in WAVE2_CARDS:
        meanings["finance"] = {"upright": finance_text(card, up), "reversed": finance_text(card, down)}
        meanings["health"] = {"upright": health_text(card, up), "reversed": health_text(card, down)}
        meanings["spirit"] = {"upright": spirit_text(card, up), "reversed": spirit_text(card, down)}
    else:
        meanings["finance"] = {"upright": TODO_STUB, "reversed": TODO_STUB}
        meanings["health"] = {"upright": TODO_STUB, "reversed": TODO_STUB}
        meanings["spirit"] = {"upright": TODO_STUB, "reversed": TODO_STUB}
...
def main():
    ...
    print(f"Wave 2 fully-authored (finance/health/spirit) cards: {len(WAVE2_CARDS)}")
